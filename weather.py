        ...
        for w in j["RESPONSE"]["RESULT"][0]["WeatherMeasurepoint"]:
            if w["Id"] == str(config["Weather"]["StationID"]):
                ...
                if "Aggregated30minutes" in observation:
                    try:
                        aggregated30 = observation["Aggregated30minutes"]
                        if "Precipitation" in aggregated30:
                            precipitation = aggregated30["Precipitation"]
                            if "Rain" in precipitation and precipitation["Rain"]:
                                precip_amount = precipitation["RainSum"]["Value"]
                            elif "Snow" in precipitation and precipitation["Snow"]:
                                precip_amount = precipitation["SnowSum"]["Solid"]["Value"]
                    except KeyError as e:
